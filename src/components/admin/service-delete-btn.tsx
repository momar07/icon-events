...
interface ServiceDeleteBtnProps {
  id: number;
  action: 'delete' | 'restore';
  locale: string;
}

export function ServiceDeleteBtn({ id, action, locale }: ServiceDeleteBtnProps) {
  const [loading, setLoading] = useState(false);
  const router = useRouter();

  const handleClick = async () => {
    if (action === 'delete') {
      const msg = locale === 'ar' ? 'هل تريد حذف هذه الخدمة؟' : 'Delete this service?';
